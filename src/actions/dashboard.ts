...
export async function getPendingTransactions(spaceId: string): Promise<PendingTransactionSummary[]> {
  ...
  const { data, error } = await supabase
    .from('transactions')
    .select(
      'id, type, status, source, description, amount_original, currency_original, confidence_score, ai_raw_interpretation, account_id, category_id, transaction_date, receipt_id, created_at, tags, is_business, life_domain',
    )
    .eq('space_id', spaceId)
  ...
  return data.map((row) => ({
    ...
    lifeDomain: row.life_domain as 'personal' | 'familiar' | 'salud' | null,
  }));
}
...
export async function getTransactionHistory(spaceId: string, limit = 50): Promise<TransactionHistoryItem[]> {
  ...
  const { data, error } = await supabase
    .from('transactions')
    .select(
      'id, type, description, amount_original, currency_original, transaction_date, tags, category_id, category:categories(name), is_business, life_domain, tax_treatment',
    )
    .eq('space_id', spaceId)
    .eq('status', 'confirmed')
    ...
    .returns<
      {
        ...
        tax_treatment: TransactionHistoryItem['taxTreatment'];
      }[]
    >();
