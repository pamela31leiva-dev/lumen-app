-- =============================================================================
-- 0023_founder_access.sql
-- Activacion de Cuentas y Niveles Avanzados: marca ciertas cuentas (la
-- fundadora y el dominio de su equipo) como is_founder_access, y de ahi en
-- adelante TODOS sus espacios (existentes y futuros) quedan is_pro=true de
-- forma automatica -- desbloquea tableros ejecutivos ampliados, proyeccion
-- de caja de largo plazo y auditoria de anomalias sin pasar por la
-- activacion manual via service_role que exige todo el resto de espacios
-- 'business' (0015).
-- =============================================================================
...
-- Nuevos espacios de una cuenta founder-access nacen ya is_pro=true --------
...
-- Si a alguien se le activa is_founder_access despues del registro, sus
-- espacios ya existentes tambien se actualizan solos ---------------------
...
-- handle_new_user (0003) marca is_founder_access desde el registro mismo,
-- para que un registro nuevo con ese correo no necesite backfill manual ---
...
-- -----------------------------------------------------------------------------
-- AUDITORIA DE ANOMALIAS (nivel avanzado / is_pro): agrega al snapshot los
-- gastos confirmados de los ultimos 90 dias que superan 2.5x el promedio de
-- su propia categoria (con al menos 4 gastos en esa categoria para tener una
-- base real de comparacion) -- deterministico, calculado en Postgres, nunca
-- una inferencia de IA. Se calcula siempre (barato, acotado); la interfaz
-- decide mostrarlo solo si el espacio es is_pro.
-- -----------------------------------------------------------------------------
...
as $$
  select jsonb_build_object(
    ...
    -- Distribucion por carpeta del mes en curso, para la Radiografia
    -- Proporcional (barra segmentada) -- suma en Postgres, nunca en el
    -- cliente ni inventada por IA.
    ...
    -- Mapa de calor de gasto por dia de la semana (0=domingo) de los
    -- ultimos 90 dias -- alimenta el heatmap de la Radiografia Proporcional.
    ...
    -- Auditoria de Anomalias (nivel avanzado): gastos que superan 2.5x el
    -- promedio de su propia categoria en los ultimos 90 dias. Se calcula
    -- siempre (acotado y barato); la interfaz decide mostrarlo solo si el
    -- espacio es is_pro.
    'anomalies', coalesce((
      select jsonb_agg(jsonb_build_object(
