-- =============================================================================
-- 0032_p7_multicurrency_assets.sql
-- Bloque P7: Consolidacion Multidivisa y Activos Alternativos
-- =============================================================================
-- Multimoneda ya tenia la mitad de su base desde 0001: accounts.currency,
-- transactions.currency_original/exchange_rate/amount_base (generada). Lo que
-- faltaba era la PIEZA DETERMINISTICA que hoy exchange_rate=1 dejaba pendiente
-- (ver comentario en actions/capture-core.ts): una tabla real de tasas de
-- cambio, consultadas por el backend (nunca inventadas por el LLM).
--
-- exchange_rates es una cache de sistema, no un dato de ningun espacio -- se
-- lee igual desde cualquier space (por eso RLS solo permite SELECT a
-- cualquier autenticado) y solo se escribe desde el servidor con la
-- service-role key (ver actions/currency.ts), nunca desde el cliente.
-- =============================================================================
