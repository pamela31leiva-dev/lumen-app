-- =============================================================================
-- 0034_p8_transaction_tax_treatment.sql
-- Bloque P8: Herencia Automatica de Clasificacion Fiscal en Movimientos
-- =============================================================================
-- Hasta ahora la Clasificacion Tributaria (0030) vivia SOLO en
-- category_fiscal_tags, y get_fiscal_summary la aplicaba via JOIN en vivo: si
-- alguien cambiaba la etiqueta de una categoria, TODO el historico cambiaba
-- de clasificacion con ella, sin aviso, y no habia forma de que UN movimiento
-- puntual tuviera un tratamiento distinto al de su categoria.
--
-- transactions.tax_treatment materializa esa herencia al momento en que el
-- movimiento se confirma (ver actions/confirm.ts): copia el tratamiento de la
-- categoria elegida, pero desde ahi es un dato propio del movimiento -- la
-- persona puede corregirlo puntualmente desde el historial (ver
-- actions/fiscal.ts, updateTransactionTaxTreatment) sin que un cambio futuro
-- en la categoria se lo pise, y sin que corregir uno solo mueva el resto del
-- historico. Es el mismo principio que exchange_rate en la propia
-- transaccion (0001): un valor fijado en su momento, no recalculado solo.
-- =============================================================================
