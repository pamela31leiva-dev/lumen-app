-- =============================================================================
-- 0033_p7_opening_balance_fx.sql
-- Bloque P7 (fix descubierto en prueba en vivo): saldo inicial en moneda
-- extranjera no se convertia a la moneda base del espacio.
-- =============================================================================
-- account_balances.current_balance sumaba a.opening_balance CRUDO (sin
-- convertir) al total en moneda base -- un bug latente desde 0001 que nunca
-- se manifestaba porque, hasta 0032 (createAccount), toda cuenta SOLO podia
-- existir en la moneda base del espacio (la "Efectivo" automatica). En cuanto
-- una cuenta puede abrirse en otra moneda, ese saldo inicial se sumaba 1:1
-- como si ya estuviera en moneda base -- ej. una cuenta de USD 500 aparecia
-- como "$500 COP" en vez de convertirse. get_financial_history (0028) tiene
-- el mismo problema de forma independiente (no lee la vista).
--
-- La solucion sigue el mismo patron que transactions.exchange_rate: la tasa
-- se fija al momento de declarar/editar el saldo inicial (updateOpeningBalance
-- en actions/accounts.ts), no se recalcula sola despues -- determinista y
-- estable, igual que el resto del esquema.
-- =============================================================================
